$(function () {
    ...
    var speed = 0.5; // initial speed of moves - increase each move
    ...
    function shakeCups() {
        ...
        for (var i = 0; i < objMovePattern.length; i++) {
            ...
            if (i == (objMovePattern.length - 1))
                TweenMax.to($('.cup-' + objMovePattern[i][2]), speed, { left: aPos[2], delay: speed * i, ease: Sine.easeOut, onComplete: unableCupClick });
            else
                TweenMax.to($('.cup-' + objMovePattern[i][2]), speed, { left: aPos[2], delay: speed * i, ease: Sine.easeOut });
        }
    }
    ...
    function clickCup(event) {
        ...
        var iCup = currentCup.attr('class').split('cup-')[1];
        ball.css({ 'left': $('.cup-' + firstCup).position().left + widthBall, 'display': 'block' });
        TweenMax.to(currentCup, 0.5, { y: -130, ease: Sine.easeIn, delay: 0.1, onComplete: showResult });
    }

    function showResult() {
        // Show the result and store it in local storage
        $('#result').css('visibility', 'visible');
        localStorage.setItem('resultVisible', 'true');
    }

    // Check local storage on page load
    if (localStorage.getItem('resultVisible') === 'true') {
        $('#result').css('visibility', 'visible');
    }

    $('#resetButton').on('click', function () {
        // Hide the result and reset local storage
        $('#result').css('visibility', 'hidden');
        localStorage.setItem('resultVisible', 'false');
    });
